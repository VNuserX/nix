...
DRIVE_NAME_2="onedrive"

# Name and locations of the passwords file
DB_FILE_NAME="db.kdbx"
LOCAL_LOCATION="/home/nuser/vault"
REMOTE_LOCATION="vault"
#####################
...
{
    DRIVE="${1}"
	output=`rclone lsl "${DRIVE}":$REMOTE_PATH 2>/dev/null`
	if [ $? -eq 3 ]; then
		unset output
		return 1
	else
...
function sync_passwords ()
{
	...
	# Storing the values so they can be used for printing and then conversion
        ...
        human_readable_remote_mtime=`get_remote_passwords_mtime ${DRIVE} 2>/dev/null`
	...
	if [ $? -ne 0 ]; then
		...
		printf "Exporting...\t"
		passwords_export
		rclone copy $LOCAL_PATH ${DRIVE}:${REMOTE}
		printf "Done!\n"
		return 0
	fi
	...
	unset human_readable_remote_mtime
        ...
        # Handle remote being newer than local
                ...
                printf "Importing...\t"
                passwords_import
				rclone copy ${DRIVE}:${REMOTE} $LOCAL_LOCATION
                printf "Done!\n"
		return 0
...
sync_passwords $DRIVE_NAME $REMOTE_LOCATION
sync_passwords $DRIVE_NAME_2 $REMOTE_LOCATION
